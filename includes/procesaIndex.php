<?php
	...
	if($total_records == 0) {
		...
		echo '<div id="menu-ordenar">
					<ul>
						<li><a href="'.RAIZ_APP.'index.php?page='.$page.'&ordenar=alfabeticamente">Alfabeticamente</a></li>
						<li><a href="'.RAIZ_APP.'index.php?page='.$page.'&ordenar=ultimas">&Uacute;ltimas</a></li>
						<li><a href="'.RAIZ_APP.'index.php?page='.$page.'&ordenar=valoradas">M&aacute;s valoradas</a></li>
					</ul>
				</div>';
		...
		$contenido = getPagination($start_with, $rows_for_page, $ordenar);
		...
		if($page >= 1) {
			if(empty($ordenado)) {
				...
				echo '<a href="index.php?page=0&ordenar='.$ordenado.'">Primero</a>';
				echo '<a href="index.php?page='.($page-1).'&ordenar='.$ordenado.'"">Anterior</a>';
			}
		}
		...
		for($i=0;$i<$pages;$i++) {
			if(empty($ordenado)) {
				...
				echo '<a href="index.php?page='.$i.'&ordenar='.$ordenado.'">'.$i.'</a> |';
			}
		}
		...
		if($page < ($pages-1)) {
			if(empty($ordenado)) {
				...
				echo '<a href="index.php?page='.($page+1).'&ordenar='.$ordenado.'">Siguiente</a>';
				echo '<a href="index.php?page='.($pages-1).'&ordenar='.$ordenado.'">Ultimo</a>';
			}
		}
